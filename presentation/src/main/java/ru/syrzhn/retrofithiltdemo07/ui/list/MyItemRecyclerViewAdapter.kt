...
class MyItemRecyclerViewAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.startDateView.text = item.startDate.toDate()
        val hasLike = item.hasLike
        if (hasLike) {
            holder.bookmark.background = ContextCompat.getDrawable(context,ru.syrzhn.retrofithiltdemo07.R.drawable.bookmark_green)
        } else {
            holder.bookmark.background = ContextCompat.getDrawable(context,ru.syrzhn.retrofithiltdemo07.R.drawable.bookmark)
        }
        holder.publishDateView.text = item.publishDate.toDate()
